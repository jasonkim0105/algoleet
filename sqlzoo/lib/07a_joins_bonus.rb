...
def heart_tracks
  ...
  execute(<<-SQL)
    SELECT albums.title, COUNT(tracks.song) AS songCount
    FROM albums
    JOIN tracks ON tracks.album = albums.asin
    WHERE tracks.song LIKE ('%Heart%')
    GROUP BY albums.asin
    ORDER BY songCount DESC, albums.title ASC
  SQL
end
...
def expensive_tastes
  ...
  execute(<<-SQL)
    SELECT styles.style, (SUM(album_track_count.price) / SUM(album_track_count.countOfTracks))
    FROM styles
    JOIN (
      SELECT albums.*, COUNT(tracks.*) AS countOfTracks
      FROM albums
      JOIN tracks on tracks.album = albums.asin
    ...
    ORDER BY (SUM(album_track_count.price) / SUM(album_track_count.countOfTracks)) DESC, styles.style ASC
    LIMIT 5
